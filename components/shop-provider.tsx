...
export function ShopProvider({children}:{children:React.ReactNode}){
  ...
  useEffect(()=>{ try{setCart(JSON.parse(localStorage.getItem('gbp-cart')||'{}'));setSignedIn(localStorage.getItem('gbp-auth')==='yes')}catch{} setReady(true)},[]);
  useEffect(()=>{if(ready)localStorage.setItem('gbp-cart',JSON.stringify(cart))},[cart,ready]);
  ...
  const signIn=()=>{setSignedIn(true);localStorage.setItem('gbp-auth','yes')}; const signOut=()=>{setSignedIn(false);localStorage.removeItem('gbp-auth')};
